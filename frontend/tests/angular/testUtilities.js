...
const $scope = (controller) => {
  return {
    ...
    $watchCollection: jest.fn().mockImplementation((toCall, action) =>
      action(
        toCall({
          vm: controller,
        })
    ...
    $watch: jest.fn().mockImplementation((toCall, action) =>
      action(
        toCall({
          vm: controller,
        })
...
const $ngRedux = {
  connect: jest
    .fn()
    .mockReturnValue(() => jest.fn().mockReturnValue('unsubscribeFn')),
