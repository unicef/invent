...
class LinechartController {
  constructor($scope, $element, $timeout, $ngRedux, gettextCatalog) {
    ...
    this.processDataForCoverageChart = this.processDataForCoverageChart.bind(
      this
    )
  ...
  onInit() {
    ...
    if (this.datachooser) {
      const domainData =
        window.$nuxt.$store.getters['projects/getMapsDomainData']
      this.chartConfig = this.processDataForDomainChart(domainData)
    } else if (this.notpercentage) {
      const coverageData =
        window.$nuxt.$store.getters['projects/getCoverageData']
  ...
  watchers() {
    ...
    this.scope.$watchGroup(
      [(s) => s.vm.dataBit, (s) => s.vm.resizeCount],
      () => {
  ...
  draw({ data, chosenData, chosenLabels, labels, maxValue }) {
    ...
    const element = outer.append('svg').attr('class', 'visualization')

    element.attr('width', outerWidth).attr('height', outerHeight)
    ...
    const percScale = d3.scale
      .linear()
      .range([height - margin.top, margin.bottom])
      .domain([0, 1])

    const simpleScale = d3.scale
      .linear()
      .range([height - margin.top, margin.bottom])
      .domain([0, maxValue])
    ...
    const xAxis = d3.svg
      ...
      .tickFormat((d) => {
        const canShowDate =
          (width - margin.left - margin.right) / ((data.length - 1) * 80) >= 1.2
