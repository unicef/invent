...
class SolutionKPITests(TestProjectData, APITestCase):
    ...
    def test_solutions_kpi_two_snapshot_differences(self):
        ...
        self.sol_1 = Solution.objects.create(
            ...
            learning_investment=True, is_active=True
        )
        ...
        self.sol_1.portfolios.set(
            [self.port1_resp.json()['id'], self.port2_resp.json()['id']])
        self.sol_1.problem_statements.set([ps_1, ps_2])
        self.sol_2 = Solution.objects.create(
            name="Solution 2", phase=Solution.PHASES[1][0], open_source_frontier_tech=True, learning_investment=True, is_active=True
        )
        ...
        self.assertEqual(past_snapshot['portfolios']
                         [0]['landscape_review'], False)
        self.assertEqual(
            current_snapshot['portfolios'][0]['landscape_review'], False)

        self.assertEqual(past_snapshot['portfolios'][0]['status'],
                         current_snapshot['portfolios'][0]['status'])
        self.assertNotEqual(
            past_snapshot['portfolios'][1]['status'], current_snapshot['portfolios'][1]['status'])
        self.assertNotEqual(past_snapshot['portfolios'][1]['investment_to_date'],
                            current_snapshot['portfolios'][1]['investment_to_date'])
        self.assertNotEqual(
            past_snapshot['portfolios'][1]['solutions'], current_snapshot['portfolios'][1]['solutions'])
        self.assertNotEqual(
            past_snapshot['solutions'][1]['countries'], current_snapshot['solutions'][1]['countries'])
        self.assertNotEqual(
            past_snapshot['solutions'][1]['portfolios'], current_snapshot['solutions'][1]['portfolios'])
        self.assertEqual(past_snapshot['solutions'][1]['people_reached'], 400)
        self.assertEqual(
            current_snapshot['solutions'][1]['people_reached'], 456)
        self.assertEqual(past_snapshot['solutions'][0]['regions'],
                         [CountryOffice.REGIONS[0][0], CountryOffice.REGIONS[1][0]])
        self.assertEqual(self.sol_1.regions_display, [
                         CountryOffice.REGIONS[0][1], CountryOffice.REGIONS[1][1]])
    ...
    def test_country_inclusions_by_regions(self):
        ...
        update_country_inclusion_log_task(
        ...
        new_regional_office, _ = RegionalOffice.objects.get_or_create(
            name='RO inclusion test', is_included=True)
